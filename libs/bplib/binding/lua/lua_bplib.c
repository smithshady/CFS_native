...
static void push_flag_table(lua_State *L, uint16_t flags)
{
    ...
    lua_pushbool(L, (flags & BP_FLAG_NONCOMPLIANT) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "incomplete");
    lua_pushbool(L, (flags & BP_FLAG_INCOMPLETE) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "unreliabletime");
    lua_pushbool(L, (flags & BP_FLAG_UNRELIABLE_TIME) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "dropped");
    lua_pushbool(L, (flags & BP_FLAG_DROPPED) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "failedintegritycheck");
    lua_pushbool(L, (flags & BP_FLAG_FAILED_INTEGRITY_CHECK) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "bundletoolarge");
    lua_pushbool(L, (flags & BP_FLAG_BUNDLE_TOO_LARGE) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "routeneeded");
    lua_pushbool(L, (flags & BP_FLAG_ROUTE_NEEDED) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "storefailure");
    lua_pushbool(L, (flags & BP_FLAG_STORE_FAILURE) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "unknowncid");
    lua_pushbool(L, (flags & BP_FLAG_UNKNOWN_CID) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "sdnvoverflow");
    lua_pushbool(L, (flags & BP_FLAG_SDNV_OVERFLOW) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "sdnincomplete");
    lua_pushbool(L, (flags & BP_FLAG_SDNV_INCOMPLETE) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "activetablewrap");
    lua_pushbool(L, (flags & BP_FLAG_ACTIVE_TABLE_WRAP) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "duplicates");
    lua_pushbool(L, (flags & BP_FLAG_DUPLICATES) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "custodyfull");
    lua_pushbool(L, (flags & BP_FLAG_CUSTODY_FULL) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "unknownrec");
    lua_pushbool(L, (flags & BP_FLAG_UNKNOWNREC) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "invalidciphersuite");
    lua_pushbool(L, (flags & BP_FLAG_INVALID_CIPHER_SUITEID) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "invalidbibresulttype");
    lua_pushbool(L, (flags & BP_FLAG_INVALID_BIB_RESULT_TYPE) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "invalidbibtargettype");
    lua_pushbool(L, (flags & BP_FLAG_INVALID_BIB_TARGET_TYPE) != 0);
    lua_settable(L, -3);

    lua_pushstring(L, "failedtoparse");
    lua_pushbool(L, (flags & BP_FLAG_FAILED_TO_PARSE) != 0);
    lua_settable(L, -3);
}
...
int lbplib_route(lua_State *L)
{
    ...
    {
        lualog("incorrect number of parameters - expected %d\n", minargs);
        lua_pushbool(L, false); /* push result as fail */
        return 1;
    }
    ...
    /* Return */
    lua_pushbool(L, status == BP_SUCCESS);
 ...
 *                                                          l is bool for logging
 *                                                          r is bool for resetting
...
int lbplib_flashsim(lua_State *L)
{
    ...
    else
    {
        ...
        if (strcmp(cmdstr, "STAT") == 0)
        {
            ...
            if (lua_isbool(L, 2))
            {
                log_stats = lua_tobool(L, 2);
                if (lua_isbool(L, 3))
                {
                    reset_stats = lua_tobool(L, 3);
                }
            }
...
int lbplib_setopt(lua_State *L)
{
    ...
    else if ((strcmp(optstr, "REQUEST_CUSTODY") == 0) && lua_isbool(L, 3))
    {
        int cstrqst = (int)lua_tobool(L, 3);
        status      = bplib_config(bplib_data->desc, BP_OPT_MODE_WRITE, BP_OPT_REQUEST_CUSTODY, &cstrqst);
    }
    else if ((strcmp(optstr, "ADMIN_RECORD") == 0) && lua_isbool(L, 3))
    {
        int admin = (int)lua_tobool(L, 3);
        status    = bplib_config(bplib_data->desc, BP_OPT_MODE_WRITE, BP_OPT_ADMIN_RECORD, &admin);
    }
    else if ((strcmp(optstr, "INTEGRITY_CHECK") == 0) && lua_isbool(L, 3))
    {
        int icheck = (int)lua_tobool(L, 3);
        status     = bplib_config(bplib_data->desc, BP_OPT_MODE_WRITE, BP_OPT_INTEGRITY_CHECK, &icheck);
    }
    else if ((strcmp(optstr, "ALLOW_FRAGMENTATION") == 0) && lua_isbool(L, 3))
    {
        int frag = (int)lua_tobool(L, 3);
        status   = bplib_config(bplib_data->desc, BP_OPT_MODE_WRITE, BP_OPT_ALLOW_FRAGMENTATION, &frag);
    }
    else if ((strcmp(optstr, "IGNORE_EXPIRATION") == 0) && lua_isbool(L, 3))
    {
        int ign = (int)lua_tobool(L, 3);
    ...
    else if ((strcmp(optstr, "CID_REUSE") == 0) && lua_isbool(L, 3))
    {
        int reuse = (int)lua_tobool(L, 3);

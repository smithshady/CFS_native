...
int32 IO_TransUdpConfigSocket(IO_TransUdpConfig_t *config, IO_TransUdp_t *udp)
{
    ...
    CFE_PSP_MemSet((void *) &udp->sockAddr, 0x0, sizeof(OS_SockAddr_t));
    CFE_PSP_MemSet((void *) &udp->srcAddr, 0x0, sizeof(OS_SockAddr_t));
    CFE_PSP_MemSet((void *) &udp->destAddr, 0x0, sizeof(OS_SockAddr_t));

    /* Save UDP Socket Addr structure */
    //udp->sockAddr.sin_family        = AF_INET;
    udp->sockAddr.AddrData.AlignU32   = uiAddr;
    //udp->sockAddr.Port          = htons(config->usPort);
                             ...
                             uint16 usPort)
{
    ...
    //udp->destAddr.sa_family        = AF_INET;
    udp->destAddr.AddrData.AlignU32   = uiAddr;
    //udp->destAddr.Port          = htons(usPort);
...
{
    socklen_t addrLen = sizeof(OS_SockAddr_t);
    int32 msgSize;

                             ...
                             uint16 bufferLength, uint32 patternBitLength)
{
    ...
    CFE_MSG_Init((void *) pIdlePacket, CFE_SB_ValueToMsgId(0x3ffU), bufferLength);
                          ...
                          TM_SDLP_ChannelConfig_t *pChannelConfig)
{
    ...
    sprintf(mutName, "TF Global VC ID %hu", gvcid);
...
int32 TM_SDLP_AddPacket(TM_SDLP_FrameInfo_t *pFrameInfo, CFE_MSG_Message_t *pPacket)
{
    ...
    iStatus = TM_SDLP_AddData(pFrameInfo, (uint8 *) pPacket,(uint16_t) length, true);

...
void OS_BSP_Lock_Impl(void)
{
    ...
    status = semTake(OS_BSP_GenericFreeRtosGlobal.AccessMutex, WAIT_FOREVER);
    if (status != OK)
    {
...
void OS_BSP_Unlock_Impl(void)
{
    ...
    status = semGive(OS_BSP_GenericFreeRtosGlobal.AccessMutex);
    if (status != OK)
    {
...
int OS_BSPMain(void)
{
     ...
     */
    OS_BSP_GenericFreeRtosGlobal.AccessMutex = semMInitialize(OS_BSP_GenericFreeRtosGlobal.AccessMutexMem,
                                                             SEM_Q_PRIORITY | SEM_INVERSION_SAFE | SEM_DELETE_SAFE);

    if (OS_BSP_GenericFreeRtosGlobal.AccessMutex == (SEM_ID)0)
    {

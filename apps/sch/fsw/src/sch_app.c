    ...
    else
    {
        ...
        */
        ...
        if (Remainder == NextEntry->Remainder)
        {
            ...
            // TODO check header update
            Status = CFE_SB_TransmitMsg((CFE_MSG_Message_t *) Message, false);
        ...
        *           Message = SCH_AppData.MessageTable[NextEntry->MessageIndex].MessageBuffer;
        *           Status = CFE_SB_TransmitMsg((CFE_MSG_Message_t *) Message);
        *           break;
        *            
        *       default:
...
int32 SCH_ValidateMessageData(void *TableData)
{
    ...
    CFE_SB_MsgId_t  MaxValue = {.Value = SCH_MDT_MAX_MSG_ID};
    CFE_SB_MsgId_t  MinValue = {.Value = SCH_MDT_MIN_MSG_ID};
    ...
    */
    for (TableIndex = 0; TableIndex < SCH_MAX_MESSAGES; TableIndex++)
    {
        ...
        else if ((MessageID.Value <= MaxValue.Value) && 
                 (MessageID.Value >= MinValue.Value))
        {
            /*

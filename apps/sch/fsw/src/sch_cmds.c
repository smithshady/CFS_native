...
int32 SCH_AppPipe(CFE_SB_Buffer_t* MessagePtr)
{
    ...
    CFE_SB_MsgId_t  MessageID = {.Value = 0};
...
int32 SCH_HousekeepingCmd(CFE_SB_Buffer_t* MessagePtr)
{
    ...
    if(SCH_VerifyCmdLength(MessagePtr, sizeof(SCH_NoArgsCmd_t)) == SCH_SUCCESS)
    {
        ...
        */
    //TODO check update header
        CFE_SB_TimeStampMsg((CFE_MSG_Message_t *) &SCH_AppData.HkPacket);
        CFE_SB_TransmitMsg((CFE_MSG_Message_t *) &SCH_AppData.HkPacket, false);
...
void SCH_NoopCmd(CFE_SB_Buffer_t* MessagePtr)
{
    if(SCH_VerifyCmdLength(MessagePtr, sizeof(SCH_NoArgsCmd_t)) != SCH_SUCCESS)
    {
        SCH_AppData.ErrCounter++;
...
void SCH_ResetCmd(CFE_SB_Buffer_t* MessagePtr)
{
    if(SCH_VerifyCmdLength(MessagePtr, sizeof(SCH_NoArgsCmd_t)) != SCH_SUCCESS)
    {
        SCH_AppData.ErrCounter++;
...
void SCH_EnableCmd(CFE_SB_Buffer_t* MessagePtr)
{
...
void SCH_DisableCmd(CFE_SB_Buffer_t* MessagePtr)
{
    bool         GoodCommand = false;
...
void SCH_EnableGroupCmd(CFE_SB_Buffer_t* MessagePtr)
{
    bool              GoodCommand = false;
...
void SCH_DisableGroupCmd(CFE_SB_Buffer_t* MessagePtr)
{
    bool              GoodCommand = false;
    uint32               TblGroupNumber = 0;
    uint32               TblMultiGroup = 0;
...
void SCH_EnableSyncCmd(CFE_SB_Buffer_t* MessagePtr)
{
    bool   GoodCommand = false;
...
void SCH_SendDiagTlmCmd(CFE_SB_Buffer_t* MessagePtr)
{
    bool              GoodCommand = false;
    uint32               TblIndex = 0;
    uint32               WordIndex = 0;
    ...
    if(SCH_VerifyCmdLength(MessagePtr, sizeof(SCH_NoArgsCmd_t)) == SCH_SUCCESS)
    {
        ...
        for (TblIndex=0; TblIndex<SCH_TABLE_ENTRIES; TblIndex++)
        {
            ...
            {
                // TODO check on the pointer conversions for both of these
                SCH_AppData.DiagPacket.EntryStates[WordIndex] |= (1 << BitIndex);
                CFE_MSG_GetMsgId((CFE_MSG_Message_t *)&SCH_AppData.MessageTable[SCH_AppData.ScheduleTable[TblIndex].MessageIndex], &(SCH_AppData.DiagPacket.MsgIDs[TblIndex]));
            }
    ...
    //TODO check update header
        ...
        CFE_SB_TransmitMsg((CFE_MSG_Message_t *) &SCH_AppData.DiagPacket, false);
...
int32 SCH_VerifyCmdLength (CFE_SB_Buffer_t* MessagePtr, uint32 ExpectedLength)
{
    int32               Status = SCH_SUCCESS;
    CFE_SB_MsgId_t      MessageID = CFE_SB_ValueToMsgId(0);
    CFE_MSG_FcnCode_t              CommandCode = 0; 

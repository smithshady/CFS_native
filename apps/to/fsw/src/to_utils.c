...
int32 TO_FindEmptyTableIndex(void)
{
    ...
    for (ii = 0; ii < TO_MAX_TBL_ENTRIES; ii++)
    {
        ...
        if (pEntry->usMsgId.Value == TO_UNUSED_ENTRY ||
            pEntry->usMsgId.Value == TO_REMOVED_ENTRY)
        {
            tableIdx = ii;
            break;
        }
                        ...
                        CFE_SB_MsgId_t  usMsgId)
{
    ...
    for (ii = 0; ii < TO_MAX_TBL_ENTRIES; ii++)
    {
        ...
        else if (pEntry->usMsgId.Value == usMsgId.Value)
        {
            tableIdx = ii;
            break;
        }
...
*******************************************************************************/
bool TO_VerifyCmdLength(CFE_SB_Buffer_t* pMsg,
...
int32 TO_SubscribeAllMsgs(void)
{
    ...
    for (ii = 0; ii < TO_MAX_TBL_ENTRIES; ++ii)
    {
        ...
        else if (pEntry->usMsgId.Value == TO_UNUSED_ENTRY)
        {
            break;
        }
    }
            ...
            if (iStatus != CFE_SUCCESS)
            {
                ...
                CFE_EVS_SendEvent(TO_INIT_ERR_EID, CFE_EVS_EventType_ERROR,
                    "TO Pipe:%s failed to subscribe to MID 0x%04x",
                     pTlmPipe->cTlmPipeName, pEntry->usMsgId.Value);
                break;
            }
            else
            {

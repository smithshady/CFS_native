...
void TO_SendDataTypePktCmd(CFE_SB_Buffer_t*);


/******************************************************************************/
/** \brief Noop Command (TO_NOOP_CC)
*******************************************************************************/
void TO_NoopCmd(CFE_SB_Buffer_t* pCmdMsg)
{
...
*******************************************************************************/
void TO_ResetCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    if (TO_VerifyCmdLength(pCmdMsg, sizeof(TO_NoArgCmd_t)))
    {
...
*******************************************************************************/
void TO_EnableOutputCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    int32 ii = 0;
    int32 routeMask = 0;
    ...
    if (TO_VerifyCmdLength(pCmdMsg, sizeof(TO_EnableOutputCmd_t)))
    {
        ...
        */
        ...
        routeMask = TO_CustomEnableOutputCmd(CFE_MSG_PTR(*pCmdMsg));
...
*******************************************************************************/
void TO_DisableOutputCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    ...
    if (TO_VerifyCmdLength(pCmdMsg, sizeof(TO_DisableOutputCmd_t)))
    {
        ...
        iStatus = TO_CustomDisableOutputCmd(CFE_MSG_PTR(*pCmdMsg));
...
*******************************************************************************/
void TO_ActivateRoutesCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    uint16 ii;
...
*******************************************************************************/
void TO_DeactivateRoutesCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    uint16 ii;
...
*******************************************************************************/
void TO_PauseOutputCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    if (TO_VerifyCmdLength(pCmdMsg, sizeof(TO_NoArgCmd_t)))
    {
        g_TO_AppData.HkTlm.usCmdCnt++;
...
*******************************************************************************/
void TO_ResumeOutputCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    if (TO_VerifyCmdLength(pCmdMsg, sizeof(TO_NoArgCmd_t)))
    {
        g_TO_AppData.HkTlm.usCmdCnt++;
...
*******************************************************************************/
void TO_AddTblEntryCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    ...
    if (TO_VerifyCmdLength(pCmdMsg, sizeof(TO_AddTblEntryCmd_t)))
    {
        ...
        /* Validate MID */
        if (pCmd->usMsgId.Value == TO_REMOVED_ENTRY || 
            pCmd->usMsgId.Value == TO_UNUSED_ENTRY)
        {
            g_TO_AppData.HkTlm.usCmdErrCnt++;
            CFE_EVS_SendEvent(TO_CMD_ERR_EID, CFE_EVS_EventType_ERROR,
...
*******************************************************************************/
void TO_RemoveTblEntryCmd(CFE_SB_Buffer_t* pCmdMsg)
{
...
*******************************************************************************/
void TO_EnableTblEntryCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    TO_MidArgCmd_t * pCmd = (TO_MidArgCmd_t *) pCmdMsg;
    TO_TableEntry_t *pEntry = NULL;
    int32 index = 0;
...
*******************************************************************************/
void TO_DisableTblEntryCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    TO_MidArgCmd_t * pCmd = (TO_MidArgCmd_t *) pCmdMsg;
    TO_TableEntry_t * pEntry = NULL;
    int32 index = 0;
...
*******************************************************************************/
void TO_EnableGroupCmd(CFE_SB_Buffer_t* pCmdMsg)
{
...
*******************************************************************************/
void TO_DisableGroupCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    TO_GroupArgCmd_t * pCmd = (TO_GroupArgCmd_t *) pCmdMsg;
    int32 iStatus = TO_SUCCESS;
...
*******************************************************************************/
void TO_EnableAllCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    int32 iStatus = TO_SUCCESS;
...
*******************************************************************************/
void TO_DisableAllCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    int32 iStatus = TO_SUCCESS;
...
*******************************************************************************/
void TO_SetRouteByMidCmd(CFE_SB_Buffer_t* pCmdMsg)
{
...
*******************************************************************************/
void TO_SetRouteByGroupCmd(CFE_SB_Buffer_t* pCmdMsg)
{
...
*******************************************************************************/
void TO_ManageTableCmd(CFE_SB_Buffer_t* pCmdMsg)
{
...
*******************************************************************************/
void TO_SetRoutePeriodCmd(CFE_SB_Buffer_t* pCmdMsg)
{
...
*******************************************************************************/
void TO_SetWakeupTimeoutCmd(CFE_SB_Buffer_t* pCmdMsg)
{
...
*******************************************************************************/
void TO_SendDataTypePktCmd(CFE_SB_Buffer_t* pCmdMsg)
{
    ...
    if (TO_VerifyCmdLength(pCmdMsg, sizeof(CFE_MSG_CommandHeader_t)))
    {
        ...
        CFE_SB_MsgId_t msg_id = {.Value = TO_DATA_TYPE_MID};
        CFE_MSG_Init(CFE_MSG_PTR(testPacket.TlmHeader),
                       msg_id,
                       sizeof(testPacket));

        CFE_SB_TimeStampMsg(CFE_MSG_PTR(testPacket.TlmHeader));
        ...
        //TODO check on update header
        CFE_SB_TransmitMsg(CFE_MSG_PTR(testPacket.TlmHeader), false);
    }
}

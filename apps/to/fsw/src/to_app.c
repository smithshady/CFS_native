...
int32 TO_InitData(void)
{
    ...
    {
        g_TO_AppData.criticalMid[ii].Value = 0;
    }
...
int32 TO_ValidateTable(void* table)
{
    ...
    for (ii = 0; ii < TO_MAX_TBL_ENTRIES; ii++)
    {
        ...
        if (entry->usMsgId.Value != TO_UNUSED_ENTRY &&
            entry->usMsgId.Value != TO_REMOVED_ENTRY)
        {
            ...
            for (jj = ii + 1; jj < TO_MAX_TBL_ENTRIES; jj++)
            {
                ...
                if (entryCmp->usMsgId.Value == TO_UNUSED_ENTRY)
                {
                    break;
                }
                else if (entryCmp->usMsgId.Value != TO_REMOVED_ENTRY &&               
                         entry->usMsgId.Value == entryCmp->usMsgId.Value)
                {
                    CFE_EVS_SendEvent(TO_TBL_ERR_EID, CFE_EVS_EventType_ERROR,
                                       "Table Validation failed. "
        ...
        else if (entry->usMsgId.Value == TO_UNUSED_ENTRY)
        {
            reachedUnused = 1;
        }
    }
    ...
    for (ii = 0; ii < TO_NUM_CRITICAL_MIDS; ii++)
    {
        ...
        if (g_TO_AppData.criticalMid[ii].Value != 0 && iStatus == TO_NO_MATCH)
        {
            CFE_EVS_SendEvent(TO_TBL_ERR_EID, CFE_EVS_EventType_ERROR,
                               "Table Validation failed. "
...
void TO_ProcessNewData(TO_TlmPipe_t *pTlmPipe, uint16 usRouteId)
{
    ...
    CFE_SB_MsgId_t          usMsgId = {.Value = 0};
            ...
            if (g_TO_AppData.usOutputEnabled && g_TO_AppData.usOutputActive)
            {
                ...
                else
                {
                    ...
                    if (pEntry->usState && 
                        ...
                        g_TO_AppData.routes[usRouteId].usIsEnabled)
                    {
                        CFE_MSG_GetSize(CFE_MSG_PTR(*pTlmMsg), &size);
                        iStatus = TO_CustomProcessData(CFE_MSG_PTR(*pTlmMsg), size, iTblIdx, 
                                                       usRouteId);
...
*******************************************************************************/
void TO_ProcessNewAppCmds(CFE_SB_Buffer_t* pMsg)
{
    ...
    if (pMsg != NULL)
    {
        CFE_MSG_GetFcnCode(CFE_MSG_PTR(*pMsg), &uiCmdCode);
        switch (uiCmdCode)
        {
            case TO_NOOP_CC:
                TO_NoopCmd(pMsg);
...
{
    CFE_SB_TimeStampMsg(CFE_MSG_PTR(g_TO_AppData.HkTlm.ucTlmHeader));
    // TODO verify header update setting
    CFE_SB_TransmitMsg(CFE_MSG_PTR(g_TO_AppData.HkTlm.ucTlmHeader), false);
}
...
{
    CFE_SB_TimeStampMsg(CFE_MSG_PTR(g_TO_AppData.OutData.ucTlmHeader));
    // TODO verify header update setting
    CFE_SB_TransmitMsg(CFE_MSG_PTR(g_TO_AppData.OutData.ucTlmHeader), false);
}

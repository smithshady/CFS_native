...
void Ut_CFE_SB_Reset(void)
{
    ...
    Ut_CFE_SB_SetFunctionHook(UT_CFE_SB_GETTOTALMSGLENGTH_INDEX, &Ut_CFE_MSG_GetSizeHook);
...
void Ut_CFE_SB_SetFunctionHook(uint32 Index, void *FunPtr)
{
    ...
    else if (Index == UT_CFE_SB_GETTOTALMSGLENGTH_INDEX)  { Ut_CFE_SB_HookTable.CFE_MSG_GetSize = FunPtr; }
...
uint16 CFE_MSG_GetSize(CFE_SB_Buffer_t MsgPtr)
{
    /* Check for Function Hook */
    if (Ut_CFE_SB_HookTable.CFE_MSG_GetSize)
        return Ut_CFE_SB_HookTable.CFE_MSG_GetSize(MsgPtr);

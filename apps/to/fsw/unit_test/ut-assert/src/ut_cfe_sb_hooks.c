...
{
    UtList_Add(&MsgQueue, MsgPtr, CFE_MSG_GetSize(MsgPtr), 0);

    UtPrintf("PKT: ");
    UtPrintx(MsgPtr, (uint16)(CFE_MSG_GetSize(MsgPtr)));
    return CFE_SUCCESS;
}
...
void Ut_CFE_SB_AddMsgToPipe(void *MsgPtr, CFE_SB_PipeId_t PipeId)
{
    if (PipeTable[PipeId].InUse == true) {
        UtList_Add(&PipeTable[PipeId].MsgQueue, MsgPtr, CFE_MSG_GetSize((CFE_SB_Buffer_t)MsgPtr), 0);
    }
    else {
...
uint16 Ut_CFE_SB_GetUserDataLengthHook(CFE_SB_Buffer_t MsgPtr)
{
    ...
    TotalMsgSize = CFE_MSG_GetSize(MsgPtr);
    HdrSize = CFE_SB_MsgHdrSize(MsgId);
...
uint16 Ut_CFE_MSG_GetSizeHook(CFE_SB_Buffer_t MsgPtr)
{
#ifdef MESSAGE_FORMAT_IS_CCSDS

...
extern void TO_SendDataTypePktCmd(CFE_SB_Buffer_t pMsg);
...
void Test_TO_RcvMsg_BadMsg(void)
{
    ...
    CFE_SB_Buffer_t pMsg = (CFE_SB_Buffer_t) &msg;
...
void Test_TO_RcvMsg_Wakeup(void)
{
    ...
    CFE_SB_Buffer_t pMsg = (CFE_SB_Buffer_t) &cmdMsg;
...
void Test_TO_AddTblEntryCmd(void)
{
    ...
    UtAssert_True(g_TO_AppData.pConfigTable->entries[3].usMsgId.Value == CFE_EVS_EVENT_MSG_MID,
...
void Test_TO_RemoveTblEntryCmd(void)
{
    ...
    UtAssert_True(g_TO_AppData.pConfigTable->entries[0].usMsgId.Value == TO_REMOVED_ENTRY,

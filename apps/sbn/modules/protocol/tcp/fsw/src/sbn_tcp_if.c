...
static SBN_Status_t ConfAddr(OS_SockAddr_t *Addr, const char *Address)
{
    ...
    {
        EVSSendErr(SBN_TCP_SOCK_EID, "setting address port failed (Port=%ld)", Port);
        return SBN_ERROR;
    } /* end if */
...
static void CheckNet(SBN_NetInterface_t *Net)
{
    ...
    for (PeerIdx = 0; PeerIdx < Net->PeerCnt; PeerIdx++)
    {
        ...
        if (PeerData->ConnectOut && !Peer->Connected)
        {
            ...
            {
                EVSSendInfo(SBN_TCP_DEBUG_EID, "connecting to peer (PeerData=0x%lx, ProcessorID=%lu)",
                            (unsigned long int)PeerData, (unsigned long int) Peer->ProcessorID);

                PeerData->LastConnectTry.ticks = LocalTime.ticks;
                ...
                EVSSendInfo(SBN_TCP_DEBUG_EID, "CPU %lu connected", (long unsigned int) Peer->ProcessorID);
...
static SBN_Status_t Send(SBN_PeerInterface_t *Peer, SBN_MsgType_t MsgType, SBN_MsgSz_t MsgSz, void *Msg)
{
    ...
    {
        EVSSendInfo(SBN_TCP_DEBUG_EID, "CPU %lu failed to write, disconnected", (long unsigned int) Peer->ProcessorID);
...
static SBN_Status_t PollPeer(SBN_PeerInterface_t *Peer)
{
    ...
    {
        EVSSendInfo(SBN_TCP_DEBUG_EID, "CPU %lu timeout, disconnected", (long unsigned int) Peer->ProcessorID);
        ...
        if (OS_SelectFdIsSet(&FdSet, Conn->Socket))
        {
            ...
            if (ToRead)
            {
                ...
                if (Received <= 0)
                {
                    ...
                    EVSSendInfo(SBN_TCP_DEBUG_EID, "CPUID %lu body recv failed, disconnected", (unsigned long int) ProcessorID);

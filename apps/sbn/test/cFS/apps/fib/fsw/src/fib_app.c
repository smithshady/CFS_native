...
void FIB_AppMain(void)
{
    ...
    CFE_SB_TransmitMsg((CFE_MSG_Message_t *)&tlm); /* send out the 1, 1; as is tradition */
    CFE_SB_TransmitMsg((CFE_MSG_Message_t *)&tlm);
    ...
    while (CFE_ES_RunLoop(&Fib_AppData.RunStatus) == true)
    {
        ...
        status = CFE_SB_ReceiveBuffer(&MsgPtr, Fib_AppData.Pipe, CFE_SB_PEND_FOREVER);

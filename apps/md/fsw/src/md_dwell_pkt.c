...
void MD_SendDwellPkt(uint16 TableIndex)
{
    ...
    */
    ...
    CFE_MSG_SetSize(CFE_MSG_PTR(PktPtr->TlmHeader), DwellPktSize);

    /*
    ** Send dwell telemetry packet.
    */
    CFE_SB_TimeStampMsg(CFE_MSG_PTR(PktPtr->TlmHeader));
    CFE_SB_TransmitMsg(CFE_MSG_PTR(PktPtr->TlmHeader), true);
}

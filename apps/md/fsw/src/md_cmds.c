    ...
    { /* Handle Nominal Case */
        ...
        else
        {
            ...
            CFE_EVS_SendEvent(MD_START_DWELL_ERR_EID, CFE_EVS_EventType_ERROR,
                              "Start Dwell Table for mask 0x%04X failed for %d of %d tables", (unsigned int)Start->TableMask,
    ...
    */
    {
        ...
        else
        {
            CFE_EVS_SendEvent(MD_SET_SIGNATURE_ERR_EID, CFE_EVS_EventType_ERROR,
                              "Failed to set signature for Dwell Tbl#%d. Update returned 0x%08X", TblId, (unsigned int)Status);

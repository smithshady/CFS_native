...
CFE_Status_t MD_InitSoftwareBusServices(void)
{
    ...
    */
    CFE_MSG_Init(CFE_MSG_PTR(MD_AppData.HkPkt.TlmHeader), CFE_SB_ValueToMsgId(MD_HK_TLM_MID), MD_HK_TLM_LNGTH);
    /*
    ** Initialize dwell packets (clear user data area)
    */
    for (TblIndex = 0; TblIndex < MD_NUM_DWELL_TABLES; TblIndex++)
    {
        CFE_MSG_Init(CFE_MSG_PTR(MD_AppData.MD_DwellPkt[TblIndex].TlmHeader),
    ...
    for (TblIndex = 0; (TblIndex < MD_NUM_DWELL_TABLES) && (TableInitValidFlag == true); TblIndex++)
    {
        ...
        if (Status < 0)
        {
            CFE_EVS_SendEvent(MD_INIT_TBL_NAME_ERR_EID, CFE_EVS_EventType_ERROR,
                              "TableName could not be made. Err=0x%08X, Idx=%d", (unsigned int)Status, TblIndex);
        ...
        if (Status < 0)
        {
            CFE_EVS_SendEvent(MD_INIT_TBL_FILENAME_ERR_EID, CFE_EVS_EventType_ERROR,
                              "TblFileName could not be made. Err=0x%08X, Idx=%d",(unsigned int) Status, TblIndex);
...
void MD_HkStatus()
{
    ...
    */
    CFE_SB_TimeStampMsg(CFE_MSG_PTR(HkPktPtr->TlmHeader));
    CFE_SB_TransmitMsg(CFE_MSG_PTR(HkPktPtr->TlmHeader), true);
}
